...
// Service for opret låneanmodnin
export const createRentalRequest = async (rentalData) => {
    const response = await fetch(${BASE_URL}/api/rentalRequests, {
        method: "POST",
        headers: {
    ...
    if (!response.ok) {
        throw new Error("Fejl ved oprettelse af låneanmodning");
    }
...
export const getAllRentalRequests = async () => {
    ...
    if (!response.ok) {
        throw new Error("Fejl ved hentning af låneanmodninger");
    }
